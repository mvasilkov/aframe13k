...
AFRAME.registerComponent('canvas-paint-wall', {
    ...
    init() {
        const canvas = document.getElementById(this.data).getContext('2d')
...
AFRAME.registerComponent('program', {
    ...
    init() {
        ...
        const button = document.getElementById('insert-coin')
        ...
        const reel = this.reel = document.getElementById('canvas-reel')
        const reelCanvas = reel.getContext('2d')
        ...
        const canvas = this.canvas = document.getElementById(this.data).getContext('2d')
    ...
    tick(time, timeDelta) {
        if (this.state == STATE_SPINNING) {
            ...
            if (this.speed[0] == 0 && this.speed[1] == 0 && this.speed[2] == 0) {
                ...
                this.state = STATE_IDLE
                if (this.money > 0) this.buttonEnable()
                else {
                    const c = document.getElementById('canvas-button').getContext('2d')
                    c.fillStyle = '#fff'
